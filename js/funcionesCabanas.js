...
function getDataCategoria() {
    $.ajax({
        type: 'GET',
        url: 'http://129.151.115.73:8080/api/Category/all',
        dataType: 'json',
...
function guardarCabana() {
    if ($('#nombreCab').val().trim() == "" || $('#marca').val().trim() == "" || $('#cuartos').val().trim() == "" || $('#descripcionC').val().trim() == "") {
        ...
        $.ajax({
            url: 'http://129.151.115.73:8080/api/Cabin/save',
            data: datosaEnviar,
            type: 'POST',
            dataType: 'json',
...
function traeEditarCabana(ide) {
    $.ajax({
        url: 'http://129.151.115.73:8080/api/Cabin/' + ide,
        type: 'GET',
        dataType: 'json',
        success: function (respuesta) {
            console.log(respuesta)
...
function editarCabana() {
    if ($('#nombreCab').val().trim() == "" || $('#marca').val().trim() == "" || $('#cuartos').val().trim() == "" || $('#descripcionC').val().trim() == "") {
        ...
        $.ajax({
            url: 'http://129.151.115.73:8080/api/Cabin/update',
            data: datosaEnviar,
            type: 'PUT',
            dataType: 'json',
            contentType: 'application/json',
