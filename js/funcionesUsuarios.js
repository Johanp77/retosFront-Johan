...
function consultarAdministradorTodo() {
    $.ajax({
        url: 'http://129.151.115.73:8080/api/Admin/all',
        type: 'GET',
        dataType: 'json',
...
function guardarAdministrador() {
    if ($('#nombreU').val().trim() == "" || $('#correoU').val().trim() == "" || $('#contrasenaU').val().trim() == "") {
        ...
        $.ajax({
            url: 'http://129.151.115.73:8080/api/Admin/save',
            data: datosaEnviar,
            type: 'POST',
            dataType: 'json',
...
function traeEditarAdministrador(ide) {
    $.ajax({
        url: 'http://129.151.115.73:8080/api/Admin/' + ide,
        type: 'GET',
        dataType: 'json',
        success: function (respuesta) {
            console.log(respuesta)
...
function editarAdministrador() {
    if ($('#nombreU').val().trim() == "" || $('#correoU').val().trim() == "" || $('#contrasenaU').val().trim() == "") {
        ...
        $.ajax({
            url: 'http://129.151.115.73:8080/api/Admin/update',
            data: datosaEnviar,
            type: 'PUT',
            dataType: 'json',
            contentType: 'application/json',
...
function ingresoGitHub() {
    $.ajax({
        url: 'http://129.151.115.73:8080/user',
        type: 'GET',
        dataType: 'json',
        success: function (respuesta) {
            console.log(respuesta)
